...
class syntax_plugin_exec extends DokuWiki_Syntax_Plugin {
    ...
    */
    ...
    function handle($match, $state, $pos, &$handler){
		echo '<b><i>handle(): state = [' . $state .']</i></b><br>';
		echo '<b><i>handle(): match = [' . $match .']</i></b><br>';
		echo '<b><i>handle(): pos =   [' . $pos .']</i></b><br>';
        switch ($state) {
   ...
   /**
    ...
    function render($mode, &$renderer, $indata) {
        if($mode == 'xhtml'){
			echo '<b><i>render(): ' . $indata[0] . '-[' . $indata[1] .']</i></b><br>';
            ...
            case DOKU_LEXER_ENTER :      
              ...
              case DOKU_LEXER_UNMATCHED : 
                ...
                // $renderer->doc .= $match . ' title=' . $match . ' class="exec">' . $match;
                $renderer->doc .= $match . ' title=' . $match . ' class="exec">' . basename($match);
                break;
 
              case DOKU_LEXER_EXIT :
                // $renderer->doc .= "</a></p></div><p>";
                $renderer->doc .= "</a>";
                break;
            }
            return true;
